// components/svg/LinkedIn.jsx

import { StripePattern } from "./StripePattern";

export default function EmailIcon({ className = "" }) {

    const pattern = StripePattern();

    return (
        <svg
            className={`stripe-animated-icon ${className}`}
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 700 700"
            onMouseEnter={() => {
                document.getElementById(`${pattern.id}-anim`)?.beginElement();
            }}
            onMouseLeave={() => {
                document.getElementById(`${pattern.id}-anim`)?.endElement();
            }}
        >
            <defs>{pattern.defs}</defs>

            {/* LinkedIn Logo */}
            <path
                d="M676.67,59.02v568.63c0,13-5.17,25.47-14.36,34.66-9.19,9.19-21.66,14.36-34.66,14.36H59.02c-13,0-25.47-5.16-34.66-14.36-9.19-9.19-14.36-21.66-14.36-34.66V59.02c0-13,5.16-25.47,14.36-34.66,9.19-9.19,21.66-14.36,34.66-14.36h568.63c13,0,25.47,5.16,34.66,14.36,9.19,9.19,14.36,21.66,14.36,34.66h0ZM206.08,264.9h-98.04v313.72h98.04v-313.72ZM214.9,157.06c.05-7.42-1.36-14.77-4.15-21.64-2.79-6.87-6.91-13.13-12.11-18.41-5.21-5.28-11.4-9.48-18.24-12.37-6.83-2.89-14.17-4.4-21.58-4.45h-1.77c-15.08,0-29.54,5.99-40.21,16.65-10.66,10.66-16.65,25.13-16.65,40.21s5.99,29.54,16.65,40.21c10.66,10.66,25.13,16.65,40.21,16.65h0c7.42.18,14.8-1.1,21.72-3.77,6.92-2.67,13.25-6.68,18.62-11.8s9.68-11.24,12.69-18.03c3-6.78,4.64-14.09,4.82-21.51v-1.77ZM578.63,388.04c0-94.31-60-130.98-119.61-130.98-19.52-.98-38.95,3.18-56.36,12.06-17.41,8.87-32.19,22.16-42.86,38.53h-2.75v-42.74h-92.16v313.72h98.04v-166.86c-1.42-17.09,3.97-34.05,14.98-47.19,11.01-13.14,26.77-21.41,43.84-23h3.73c31.18,0,54.31,19.61,54.31,69.02v168.04h98.04l.78-190.59Z"
                fill={`url(#${pattern.id})`}
                stroke="var(--accentPrimary)"
                strokeWidth="28"
            />
        </svg>
    );
}