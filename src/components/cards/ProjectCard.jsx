...
export default function ProjectCard({ title, description, button, imageSrc }) {
    return (
        <div className="flex flex-col items-center justify-left gap-10">
            <img src={imageSrc} alt={title} className="w-full h-auto rounded-lg" />
            <h2 className="text-xl text-[var(--accentPrimary)] font-semibold mt-4">{title}</h2>
            <p className="text-sm text-[var(--textPrimary)] mt-2">{description}</p>
